...
CONTENT = {
    "en": {
        ...
        "icloud_1": "There is no switch to turn on, and none to turn off. Whether iCloud carries a vault depends on <strong>where the folder is</strong>: put it in iCloud Drive and the system syncs it. No app can stop that, and Inkstone does not pretend to — to stop it, keep the folder somewhere else.",
        "icloud_2": "The one setting here is <strong>Keep files downloaded</strong>, on by default. iCloud saves disk space by evicting the contents of files you have not opened recently, leaving a placeholder behind. To Inkstone a placeholder is an empty note, so a vault that has been sitting idle can look like a vault that has lost its notes. Turn it off only on a device genuinely short of space.",
        ...
        "one_vault_h": "One repository, one vault, per device",
        "one_vault_1": "Two folders on the same device cannot both hold the same repository. Inkstone refuses it, and neither of them syncs until one gives it up.",
        "one_vault_2": "This is not a limit, it is the arrangement that has no correct behaviour: every sync makes the repository look like whichever folder ran last, so the two overwrite each other in turn. Nothing about it is recoverable by being cleverer — there is no way to tell which of two working copies is meant to be the repository.",
        "one_vault_3": "The pane names the vault that already has it and offers to move it, rather than simply refusing: a binding outlives the folder it was made for, and a refusal with no way through would leave you unable to use that repository anywhere.",
        "one_vault_4": "Two <em>devices</em> sharing one repository is the whole point, and is unaffected.",
...
CONTENT["zh"] = {'lang_label': '简体中文',
 ...
 'one_vault_h': '一个仓库，一台设备上只能有一个 vault',
 'one_vault_1': '同一台设备上，两个文件夹不能同时绑定同一个仓库。Inkstone 会拒绝，并且在其中一个放弃之前，两个都不会同步。',
 'one_vault_2': '这不是限制，而是一种<strong>没有正确行为可言</strong>的配置：每次同步都会把仓库改造成最后跑的那个文件夹的样子，于是两者轮流覆盖对方。再聪明的算法也救不了它——没有任何办法判断两个工作副本里哪一个才是仓库该有的样子。',
 'one_vault_3': '面板会指出是哪个 vault 已经占着它，并提供「改绑到这个 vault」，而不是简单拒绝：绑定的寿命会超过当初那个文件夹，一堵没有门的墙会让你在任何地方都用不了那个仓库。',
 'one_vault_4': '两台<em>设备</em>共用一个仓库正是这个功能的意义所在，完全不受影响。',
 'conflict_h': '冲突什么时候发生',
...
CONTENT["zh-Hant"] = {'lang_label': '繁體中文',
 ...
 'one_vault_h': '一個儲存庫，一台裝置上只能有一個 vault',
 'one_vault_1': '同一台裝置上，兩個資料夾不能同時繫結同一個儲存庫。Inkstone 會拒絕，並且在其中一個放棄之前，兩個都不會同步。',
 'one_vault_2': '這不是限制，而是一種<strong>沒有正確行為可言</strong>的配置：每次同步都會把儲存庫改造成最後跑的那個資料夾的樣子，於是兩者輪流覆蓋對方。再聰明的演算法也救不了它——沒有任何辦法判斷兩個工作副本裡哪一個才是儲存庫該有的樣子。',
 'one_vault_3': '面板會指出是哪個 vault 已經佔著它，並提供「改繫到這個 vault」，而不是單純拒絕：繫結的壽命會超過當初那個資料夾，一堵沒有門的牆會讓你在任何地方都用不了那個儲存庫。',
 'one_vault_4': '兩台<em>裝置</em>共用一個儲存庫正是這個功能的意義所在，完全不受影響。',
 'conflict_h': '衝突什麼時候發生',
...
def render(lang: str, site: str, asset_hash) -> str:
    ...
    blocks = [
        ...
        p(c["second_why"]),
        box(c["binding_box"]),
        f'  <h3>{c["one_vault_h"]}</h3>',
        p(c["one_vault_1"]), p(c["one_vault_2"]), p(c["one_vault_3"]), p(c["one_vault_4"]),
